...
for Article in $(ls -t $ArticlesDir/*); do
	...
	cat $HeaderFile | sed "s/XXX_TITLE_HERE_XXX/$ArticleTitle/g;s/XXX_LAST_MOD_DATE_XXX/$ArticleLastMod/g;s/XXX_DESCRIPTION_HERE_XXX/$ArticleDescr/g;s/XXX_POST_NAME_XXX/$ArticleBase/g" > $ArticleDest
	
	# Remove the TITLE: from the markdown, and then convert it to html and concatenate it into ArticleDest
	grep -v 'TITLE:' $Article | grep -v 'DESCRIPTION:' | $MarkdownPl >> $ArticleDest
